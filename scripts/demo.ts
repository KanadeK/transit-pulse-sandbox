...
const strategies = compareHeadways(line, config, [6, 8, 12]).map((result) => ({
  headwayMinutes: result.config.headwayMinutes,
  ...result.metrics,
}));
await mkdir("dist-release", { recursive: true });
await writeFile(
  "dist-release/demo-report.json",
  JSON.stringify(
    {
...
console.log(
  `Generated deterministic demo: ${baseline.metrics.trips} trips, ${baseline.metrics.boardedPassengers} boarded.`,
);
