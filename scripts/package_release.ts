...
const archivePath = join(releaseDir, `transit-pulse-sandbox-${version}-web.zip`);
...
const finished = new Promise<void>((resolve, reject) => { output.on('close', resolve); zip.on('error', reject); });
zip.pipe(output);
...
await writeFile(join(releaseDir, 'SHA256SUMS.txt'), `${checksum}  ${archivePath.replaceAll('\\', '/')}` + '\n');
