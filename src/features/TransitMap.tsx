...
export function TransitMap({ line }: { line: LineDefinition }) {
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      center: [line.stops[0].x, line.stops[0].y],
      zoom: 12,
    ...
    map.on("load", () => {
      ...
      map.addLayer({
        ...
        type: "line",
        source: "route",
        paint: { "line-color": "#68e6c5", "line-width": 5 },
      });
      map.addLayer({
        id: "stops-circle",
        type: "circle",
  ...
  return (
    <section className="map-card" aria-label="Synthetic line map">
      <div className="map" ref={element} />
      <p>
        {line.stops.length} synthetic stops · no external tiles or rider data
      </p>
    </section>
  );
}
